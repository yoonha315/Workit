...
이 버전(_fixedid)에서 바뀐 점 (그 외 로직은 원본 jaehyun_psref.py 그대로):
  - chunk_id를 "{prefix}_{장}_{절}_{조}_{항}_{호}_{목}_{세목}" 고정 8슬롯으로 생성.
    장/절/조/항/호/목/세목 중 없는 레벨은 위치 상관없이 '0'.
    조의N/호의N 가지번호는 '번호-N' 형태로 보존 (예: 제7조의2 -> '7-2').
  - 목/세목 chunk 자체는 원본처럼 그대로 분리해서 만든다 (검색 단위를 목까지
    쪼개는 옵션 1을 유지하는 것 — HoRAG 검색 단위 확장 여부는 별개의 실험 문제).
  - PYG(예규)는 조가 없으므로 조 슬롯은 항상 0, 절이 anchor 역할을 하고
    항(=조문 내 clause 번호) / 호(=가나다 item) / 목(=item 밑 숫자) / 세목(=목 밑
    가나다) 순으로 슬롯을 채움.
  - parse_pyg / split_direct_ho 안에서 수동 문자열 결합으로 만들던 chunk_id를
    전부 make_chunk_id() 호출로 교체. lookup()의 fallback id 생성도 동일하게 교체.
  - 파싱 로직(항/호/목/세목 분리, cross_refs 패턴, registry 키 구조, dedupe,
    필터링)은 원본과 완전히 동일 — id 생성 지점만 바꿨다.
...
사용법:
    ...
    python yoonha_psref_fixedid.py
"""
...
def make_chunk_id(prefix: str, jo: int | None = None, hang: int | None = None,
                  ho: int | str | None = None, mok: str | None = None,
                  jo_ui: int | None = None, jang: int | None = None,
                  ho_ui: int | None = None, jeol: int | None = None,
                  semok: str | None = None) -> str:
    """
...
def parse_pyg(lines: list[tuple[str, str]], prefix: str = "PYG",
    ...
    def get_chunk_id():
        return make_chunk_id(prefix, jang=_jang_v(), jeol=_jeol_v(),
                              hang=_hang_v(), ho=cur_item)
    ...
    def flush():
        ...
        if cur_item and len(mok_splits) > 1:
            ...
            while j < len(mok_splits) - 1:
                ...
                mok_chunk_id = make_chunk_id(prefix, jang=_jang_v(), jeol=_jeol_v(),
                                              hang=_hang_v(), ho=cur_item, mok=mok_num)
                ...
                if len(semok_splits) > 1:
                    ...
                    while k < len(semok_splits) - 1:
                        ...
                        semok_chunk_id = make_chunk_id(prefix, jang=_jang_v(), jeol=_jeol_v(),
                                                        hang=_hang_v(), ho=cur_item, mok=mok_num,
                                                        semok=semok_char)
                        articles.append({
        ...
        while j < len(ho_splits) - 1:
            ...
            if len(mok_splits) > 1:
                ...
                while k < len(mok_splits) - 1:
                    ...
                    mok_chunk_id = make_chunk_id(prefix, jo, ho=ho_num, ho_ui=ho_ui, mok=mok_char, jo_ui=jo_ui, jang=jang, jeol=jeol)
                    result.append({
                        "chunk_id": mok_chunk_id,
...
def lookup(registry: dict, target_prefix: str,
    ...
    if key in reg:
        return reg[key]
    return make_chunk_id(target_prefix, jo=jo, jo_ui=jo_ui, hang=hang, ho=ho, mok=mok, semok=semok)
